...
class ConfigStore {
  ...
  async getAdminSnapshot() {
    const [settingsRows, userRows, appRows, iosRows, androidRows] = await Promise.all([
      this.db.all('SELECT key, value FROM admin_settings ORDER BY key ASC'),
      this.db.all(
        `
      ...
      this.db.all(
        `
        SELECT app_id, bundle_id, team_id, key_id, key_path, production, created_at, updated_at
        FROM app_ios
        ORDER BY app_id ASC
      `
      ),
      this.db.all(
        `
        SELECT app_id, service_account_path, created_at, updated_at
        FROM app_android
        ORDER BY app_id ASC
      `
      ),
    ...
    return {
      ...
      appAndroid: androidRows.map((row) => ({
        appId: row.app_id,
        serviceAccountPath: row.service_account_path,
        createdAt: row.created_at,
        updatedAt: row.updated_at,
      })),
    };
