...
class ConfigStore {
  constructor({ databasePath, keysDir }) {
    this.databasePath = databasePath;
    this.keysDir = keysDir || null;
    this.db = null;
  }
  ...
  async getAppConfig(appId) {
    ...
    const ios = app.ios
      ? {
        ...app.ios,
        keyPath: this.resolveKeyPath(app.ios.keyPath),
      }
      : null;
    const android = app.android
      ? {
        ...app.android,
        serviceAccountPath: this.resolveKeyPath(app.android.serviceAccountPath),
      }
      : null;
    ...
    return {
      ...
      ios: ios && this.isReadableFile(ios.keyPath) ? ios : null,
      android: android && this.isReadableFile(android.serviceAccountPath) ? android : null,
    };
  }
  ...
  resolveKeyPath(storedPath) {
    if (!storedPath || typeof storedPath !== 'string') {
      return null;
    }
    if (this.isInlineKeyValue(storedPath)) {
      return storedPath;
    }
    ...
    return path.join(this.keysDir, storedPath);
  }

  isReadableFile(filePath) {
    if (!filePath) return false;
    if (this.isInlineKeyValue(filePath)) {
      return true;
    }
    try {
