...
class ConfigStore {
  ...
  async createNotifyLog(data) {
    return this.prisma.notifyLog.create({ data });
  }

  async listNotifyLogs({ limit = 200, offset = 0, appId } = {}) {
    const where = appId ? { appId } : undefined;
    return this.prisma.notifyLog.findMany({
      where,
  ...
  async countNotifyLogs({ appId } = {}) {
    const where = appId ? { appId } : undefined;
    return this.prisma.notifyLog.count({ where });
  }

  async clearNotifyLogs() {
    return this.prisma.notifyLog.deleteMany({});
  }

  async cleanupNotifyLogs(olderThanMs) {
