...
async function saveKeyFile({ buffer, keysDir, prefix, appId, ext }) {
  const safeId = appId.replace(/[^a-zA-Z0-9._-]/g, '_');
  const suffix = crypto.randomBytes(6).toString('hex');
  const fileName = `${prefix}_${safeId}_${Date.now()}_${suffix}${ext}`;
...
function createAdminRouter({ store, sessionSecret, keysDir }) {
  ...
  router.post('/apps/:appId/apns', upload.single('apnsKey'), async (req, res, next) => {
    try {
      ...
      if (req.file && req.file.buffer) {
        ...
        await cleanupKeyFile(app.ios ? app.ios.keyPath : null, keysDir);
      } else if (!keyPath) {
  ...
  router.post('/apps/:appId/apns/delete', async (req, res, next) => {
    try {
      ...
      await cleanupKeyFile(app.ios.keyPath, keysDir);
  ...
  router.post('/apps/:appId/fcm', upload.single('fcmKey'), async (req, res, next) => {
    try {
      ...
      if (req.file && req.file.buffer) {
        ...
        serviceAccountPath = await saveKeyFile({
          buffer: req.file.buffer,
          keysDir,
          prefix: 'fcm',
          appId: app.appId,
        ...
        await cleanupKeyFile(app.android ? app.android.serviceAccountPath : null, keysDir);
  ...
  router.post('/apps/:appId/fcm/delete', async (req, res, next) => {
    try {
      ...
      await cleanupKeyFile(app.android.serviceAccountPath, keysDir);
