    ...
    ? `
      <nav class="nav">
        ...
        <div class="links">
          ...
          <a href="${baseUrl}/database">Database</a>
...
function formatSettingValue(key, value, revealSecrets) {
  if (value === null || value === undefined) return '';
  const text = String(value);
  if (!revealSecrets && isSensitiveSettingKey(key)) {
    return maskSecret(text);
  }
  return text;
}

function formatSecretValue(value, revealSecrets) {
  if (value === null || value === undefined) return '';
  const text = String(value);
  return revealSecrets ? text : maskSecret(text);
}

function summarizeStored(value) {
  if (!value) return '';
  const text = String(value);
  return `stored (${text.length} chars)`;
}

function summarizeServiceAccount(value) {
  if (!value) return '';
  const text = String(value);
  try {
    const parsed = JSON.parse(text);
    const parts = [];
    if (parsed.client_email) parts.push(parsed.client_email);
    if (parsed.project_id) parts.push(parsed.project_id);
    if (parts.length > 0) {
      return parts.join(' | ');
    }
  } catch (_) {
...
function renderDatabasePage({ baseUrl, snapshot, revealSecrets, allowSecretView, message }) {
  const settingsRows = snapshot.settings.length > 0
    ? snapshot.settings
      .map((setting) => `
          <tr>
            <td>${escapeHtml(setting.key)}</td>
            <td><code>${escapeHtml(formatSettingValue(setting.key, setting.value, revealSecrets))}</code></td>
          </tr>
        `)
      .join('')
    : '<tr><td colspan="2">No settings found.</td></tr>';

  const userRows = snapshot.users.length > 0
    ? snapshot.users
      .map((user) => `
          <tr>
            ...
            <td>${escapeHtml(formatTimestamp(user.updatedAt))}</td>
          </tr>
        `)
      .join('')
    : '<tr><td colspan="4">No admin users found.</td></tr>';

  const appRows = snapshot.apps.length > 0
    ? snapshot.apps
      .map((app) => `
          <tr>
            <td>${escapeHtml(app.appId)}</td>
            <td>${escapeHtml(app.displayName || '')}</td>
            <td><code>${escapeHtml(formatSecretValue(app.apiSecret, revealSecrets))}</code></td>
            <td>${app.enabled ? 'yes' : 'no'}</td>
            <td>${escapeHtml(formatTimestamp(app.createdAt))}</td>
            <td>${escapeHtml(formatTimestamp(app.updatedAt))}</td>
          </tr>
        `)
      .join('')
    : '<tr><td colspan="6">No apps found.</td></tr>';

  const iosRows = snapshot.appIos.length > 0
    ? snapshot.appIos
      .map((app) => `
          <tr>
            <td>${escapeHtml(app.appId)}</td>
            <td>${escapeHtml(app.bundleId)}</td>
            <td>${escapeHtml(app.teamId)}</td>
            <td>${escapeHtml(app.keyId)}</td>
            <td>${app.production ? 'yes' : 'no'}</td>
            <td>${escapeHtml(summarizeStored(app.keyPath))}</td>
            <td>${escapeHtml(formatTimestamp(app.createdAt))}</td>
            <td>${escapeHtml(formatTimestamp(app.updatedAt))}</td>
          </tr>
        `)
      .join('')
    : '<tr><td colspan="8">No iOS configs found.</td></tr>';

  const androidRows = snapshot.appAndroid.length > 0
    ? snapshot.appAndroid
      .map((app) => `
          <tr>
            <td>${escapeHtml(app.appId)}</td>
            <td>${escapeHtml(summarizeServiceAccount(app.serviceAccountPath))}</td>
            <td>${escapeHtml(formatTimestamp(app.createdAt))}</td>
            <td>${escapeHtml(formatTimestamp(app.updatedAt))}</td>
          </tr>
        `)
      .join('')
    : '<tr><td colspan="4">No Android configs found.</td></tr>';

  const stats = `Settings: ${snapshot.settings.length} | Users: ${snapshot.users.length} | Apps: ${snapshot.apps.length}`;
  const revealLabel = revealSecrets ? 'Hide secrets' : 'Show secrets';
  const revealUrl = revealSecrets ? `${baseUrl}/database` : `${baseUrl}/database?reveal=true`;
  const revealAction = allowSecretView
    ? `<a class="btn ghost" href="${revealUrl}">${revealLabel}</a>`
    : '<div class="hint">Set ADMIN_ALLOW_SECRET_VIEW=true to enable secret reveal.</div>';
  const rawPanel = revealSecrets
    ? `
      <div class="panel">
        <h3>Raw JSON (includes secrets)</h3>
        <pre class="code-block">${escapeHtml(JSON.stringify(snapshot, null, 2))}</pre>
      </div>
    `
    : '';
  ...
  return renderLayout({ title: 'Database', baseUrl, user: true, message, content });
}

function createAdminRouter({ store, sessionSecret }) {
  ...
  router.get('/database', async (req, res, next) => {
    try {
      ...
      const revealRequested = req.query.reveal === 'true';
      const allowSecretView = process.env.ADMIN_ALLOW_SECRET_VIEW === 'true';
      const revealSecrets = allowSecretView && revealRequested;
      const message = parseMessage(req) || (
        revealRequested && !allowSecretView
          ? { type: 'error', text: 'Secret view disabled. Set ADMIN_ALLOW_SECRET_VIEW=true.' }
          : null
      );
      return res.send(renderDatabasePage({
        baseUrl: req.baseUrl,
        snapshot,
        revealSecrets,
        allowSecretView,
  ...
  router.post('/apps/:appId/apns', upload.single('apnsKey'), async (req, res, next) => {
    try {
      ...
      if (!isInlineKeyValue(keyPath)) {
        return res.redirect(303, `${req.baseUrl}/apps/${encodeURIComponent(app.appId)}/apns?error=APNs%20key%20must%20be%20uploaded`);
      }
  ...
  router.post('/apps/:appId/fcm', upload.single('fcmKey'), async (req, res, next) => {
    try {
      ...
      if (!isInlineKeyValue(serviceAccountPath)) {
        return res.redirect(303, `${req.baseUrl}/apps/${encodeURIComponent(app.appId)}/fcm?error=Service%20account%20must%20be%20uploaded`);
      }
