...
import { invalidateApnsProvider } from '../apns.js';
import { invalidateFcmMessaging } from '../fcm.js';
...
function renderApnsPage({ baseUrl, app, message }) {
  const envValue = app.ios && app.ios.production ? 'production' : 'sandbox';
  const envLabel = envValue === 'production'
    ? 'Production (App Store / TestFlight)'
    : 'Sandbox (Development build)';
  const envClass = envValue === 'production' ? 'ok' : 'warn';
  const content = `
    <div class="hero">
      <div>
        <h1>APNs Config</h1>
    ...
    <div class="panel">
      <form method="POST" enctype="multipart/form-data" action="${baseUrl}/apps/${encodeURIComponent(app.appId)}/apns">
        ...
        <div class="form-group">
          <label for="apnsEnv">APNs Environment</label>
          <select id="apnsEnv" name="apnsEnv" required>
            <option value="production" ${envValue === 'production' ? 'selected' : ''}>Production (App Store / TestFlight)</option>
            <option value="sandbox" ${envValue === 'sandbox' ? 'selected' : ''}>Sandbox (Development build)</option>
          </select>
          <div class="hint">Yanlış ortam seçimi iOS tokenlarını invalid yapar.</div>
        </div>
...
function renderLogsPage({ baseUrl, logs, total, limit, apps, message }) {
  const appMap = new Map((apps || []).map((app) => [app.appId, app.displayName]));
  const entries = logs
    .map((log) => {
      const appLabel = log.appId ? (appMap.get(log.appId) || log.appId) : 'unknown';
      const platform = log.platform || '-';
      const tokens = log.tokensCount !== null && log.tokensCount !== undefined ? log.tokensCount : '-';
      const statusTag = log.success
        ? '<span class="tag ok">OK</span>'
        : '<span class="tag warn">Fail</span>';
      const result = log.result || null;
      const ios = result && result.ios ? result.ios : null;
      const android = result && result.android ? result.android : null;
      const sent = (ios ? ios.sent : 0) + (android ? android.sent : 0);
      const failed = (ios ? ios.failed : 0) + (android ? android.failed : 0);
      const invalid = (ios ? ios.invalidTokensCount : 0) + (android ? android.invalidTokensCount : 0);
      const duration = log.durationMs !== null && log.durationMs !== undefined ? `${log.durationMs}ms` : '-';
      const detailPayload = {
      ...
      return `
        <details>
          <summary>
            ...
            invalid:${escapeHtml(invalid)}
            ${statusTag}
          </summary>
          <div class="log-meta">
            requestId: ${escapeHtml(log.requestId || '-')} |
            status: ${escapeHtml(log.status)} |
            duration: ${escapeHtml(duration)}
          </div>
          <div class="log-meta">
            ip: ${escapeHtml(log.clientIp || '-')} |
            client: ${escapeHtml(log.clientName || '-')} |
            ua: ${escapeHtml(log.userAgent || '-')}
          </div>
  ...
  const content = `
    <div class="hero">
      <div>
        <h1>Notify Logs</h1>
        <p>Last ${limit} entries. Total: ${total}.</p>
      </div>
      <div class="inline-actions">
    ...
    <div class="panel">
      ${entries || '<div class="muted">No logs yet.</div>'}
    </div>
  `;

  return renderLayout({ title: 'Notify Logs', baseUrl, user: true, message, content });
}
...
function createAdminRouter({ store, sessionSecret }) {
  ...
  router.get('/logs', async (req, res, next) => {
    try {
      ...
      const appId = normalizeText(req.query.appId);
      const [logs, total, apps] = await Promise.all([
        store.listNotifyLogs({ limit, appId: appId || undefined }),
        store.countNotifyLogs({ appId: appId || undefined }),
        store.listApps(),
      ]);
      const message = parseMessage(req);
      return res.send(renderLogsPage({
        baseUrl: req.baseUrl,
        logs,
        total,
        limit,
  ...
  router.post('/logs/clear', async (req, res, next) => {
    try {
      await store.clearNotifyLogs();
      return res.redirect(303, `${req.baseUrl}/logs?success=Logs%20cleared`);
    } catch (error) {
      return next(error);
    }
  });
  ...
  router.post('/apps/:appId/apns', upload.single('apnsKey'), async (req, res, next) => {
    try {
      ...
      const envRaw = normalizeText(req.body.apnsEnv).toLowerCase();
      const production = envRaw
        ? envRaw === 'production'
        : (req.body.production !== undefined ? Boolean(req.body.production) : (app.ios ? app.ios.production : false));
      ...
      invalidateApnsProvider(app.appId);
  ...
  router.post('/apps/:appId/fcm', upload.single('fcmKey'), async (req, res, next) => {
    try {
      ...
      await invalidateFcmMessaging(app.appId);
